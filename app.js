...
function App() {
  ...
  const themes = {
    Books: ["library", "novel", "book", "shelves", "author", "writer", "prose", "plot", "trope", "publish"],
    Animals: ["lion", "tiger", "elephant", "giraffe", "kangaroo", "panda", "dolphin", "penguin", "zebra", "monkey"],
    Clothes: ["shirt", "pants", "jacket", "hat", "scarf", "gloves", "dress", "socks", "shoes", "sweater"]
  };

  const [selectedTheme, setSelectedTheme] = useState("Books");

  
  const initialWords = themes[selectedTheme];
  ...
  const handleThemeChange = (newTheme) => {
    ...
    const newWords = themes[newTheme];
    setWords(newWords);
  ...
  return (
    <div>
      <h1>Scramble Game</h1>
      <label id="themeprompt">Choose a theme:</label>
    <select value={selectedTheme} onChange={(e) => handleThemeChange(e.target.value)} disabled={gameOver}>
